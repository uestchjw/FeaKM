...
#! 划分train、val数据集
def train_val_split(data_path:str,
    ...
    Num_list = list(np.arange(1,num + 1))
    
    train_split = random.sample(Num_list, math.floor(num * train_proportion))
    val_split = list(set(Num_list) - set(train_split))
    
    #* 排序
    train_split = sorted(train_split)
    val_split = sorted(val_split)
    assert len(train_split) + len(val_split) == num
