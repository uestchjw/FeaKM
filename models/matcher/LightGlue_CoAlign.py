...
#! CoAlign的数据集格式:
# data_dict:
    # "feature":       (b, 2, 128, 100, 250)
    # "cls_preds":     (b, 2, 2, 100, 250)
    # "reg_preds":     (b, 2, 14, 100, 250)
    # "pair_t_matrix": (b, 1, 5, 5, 4, 4)
class LightGlue_CoAlign(nn.Module):
    default_conf = {
    ...
    def forward(self, data: dict) -> dict:
        ...
        kpts0, kpts1 = data["keypoints0"], data["keypoints1"] #! [b, 128, 2]
        ...
        else: #! 这里我做了修改: 需要给出image_size
            size0 = data["feature"][:,0,...].shape[-2:]
            size1 = data["feature"][:,1,...].shape[-2:]
        ...
        desc0 = data["descriptors0"].contiguous() #! [b, 128, 256]
        desc1 = data["descriptors1"].contiguous() #! [b, 128, 256]
        ...
        encoding0 = self.posenc(kpts0) #! [2, b, 1, 128, 64]
        encoding1 = self.posenc(kpts1) #! [2, b, 1, 128, 64]
        ...
        for i in range(self.conf.n_layers):
            ...
            if do_early_stop: #! False
                assert b == 1
                token0, token1 = self.token_confidence[i](desc0, desc1)
                if self.check_if_stop(token0[..., :m, :], token1[..., :n, :], i, m + n):
                    break
            if do_point_pruning: #! False
                assert b == 1
        ...
        desc0, desc1 = desc0[..., :m, :], desc1[..., :n, :] #! [b, 128, 256], [b, 128, 256]
        scores, _ = self.log_assignment[i](desc0, desc1) #! [16, 129, 129]
    ...
    def loss(self, pred, data):
        ...
        #* pred:
        # keypoints0 torch.Size([16, 100, 2])
        # keypoint_scores0 torch.Size([16, 100])
        # descriptors0 torch.Size([16, 100, 256])
        # keypoints1 torch.Size([16, 100, 2])
        # keypoint_scores1 torch.Size([16, 100])
        # descriptors1 torch.Size([16, 100, 256])
        # matches0 torch.Size([B, 100])
        # matches1 torch.Size([B, 100])
        # matching_scores0 torch.Size([16, 100])
        # matching_scores1 torch.Size([16, 100])
        # ref_descriptors0 torch.Size([16, 9, 100, 256])
        # ref_descriptors1 torch.Size([16, 9, 100, 256])
        # log_assignment torch.Size([16, 101, 101])
        # prune0 torch.Size([16, 100])
        # prune1 torch.Size([16, 100])
        # gt_assignment torch.Size([16, 100, 100])
        # gt_reward torch.Size([16, 100, 100])
        # gt_matches0 torch.Size([16, 100])
        # gt_matches1 torch.Size([16, 100])
        # gt_matching_scores0 torch.Size([16, 100])
        # gt_matching_scores1 torch.Size([16, 100])
        # gt_proj_0to1 torch.Size([16, 100, 2])
        # gt_proj_1to0 torch.Size([16, 100, 2])

        
        sum_weights = 1.0
        # todo: 这里传入的是data还是pred?
        ...
        # num_matchable: [7., 7., 1., 9., 3., 1., 2., 6., 6., 4., 8., 1., 5., 6., 3., 4.]
        if self.training:
